...
from app.core.security.token_helpers.base import BaseTokenHelper
from app.core.security.token_repositories import TokensRepository
...
from app.core.security.tokens import AccessJWToken, RefreshJWToken

_guest_payload = Payload(user_id=0, user_role=UserRole.guest)


class BaseJWTokenHelper(
    BaseTokenHelper[JWTokenReadDTO, Payload],
):
    @classmethod
    def _get_payload_from_state(cls, request: Request) -> Payload:
    ...
    @classmethod
    def _delete_payload_from_state(cls, request: Request) -> None:
        request.state.payload = _guest_payload

    @classmethod
    def _get_tokens_from_state(cls, request: Request) -> JWTokenReadDTO:
    ...
    @classmethod
    def _delete_tokens_from_state(cls, request: Request) -> None:
...
class JWTokenHelper(
    ...
    @classmethod
    @override
    async def validate_tokens(cls, request: Request) -> JWTokenReadDTO:
        tokens = JWTokenReadDTO(
        ...
        if tokens.access_token is None:
            ...
            payload = await cls._read_token(refresh_token)

            updated_tokens = await cls._create_tokens(
    ...
    async def generate_tokens(
        ...
        created_tokens = await cls._create_tokens(
            access_token=AccessJWToken(),
    ...
    async def update_tokens(
        ...
        if await cls._is_user_initiator(payload, updated_payload.user_id):
            updated_base_tokens = await cls._create_tokens(
                access_token=AccessJWToken(),
                refresh_token=RefreshJWToken(),
    ...
    async def clear_tokens(
        ...
        if user_id is None or await cls._is_user_initiator(payload, user_id):
